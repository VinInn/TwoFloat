...
More precise algoritms can be selected by defining macro "*TWOFLOAT_PRECISE_SUM*" (Algo 6 for Sum), "*TWOFLOAT_PRECISE_MULT*" (Algo 12 for Multiplication) and
...
### Debugging aids
Sometimes the results are not those expected, in particular when using TwoFloat in an external software package. This is usually due to implicit assumptions that the type is a C++ floating point (with a IEEE754 format). Missing specializations are of course a major issue. 
